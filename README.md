...
COMMENT ON COLUMN medications.frequency IS 'x раз в день';
...
```
## Представления
### Представление 1: Кол-во пациентов закреплённых за врачём
```sql
...
```
### Представление 2: Заполненность палат в отделении
```sql
CREATE VIEW department_occupancy AS
SELECT 
    d.department_id,
    d.name AS department_name,
    h.name AS hospital_name,
    COUNT(w.ward_id) AS total_wards,
    SUM(w.capacity) AS total_capacity,
    COUNT(p.patient_id) AS current_patients,
    ROUND(COUNT(p.patient_id) * 100.0 / NULLIF(SUM(w.capacity), 0), 2) AS occupancy_percentage
FROM departments d
JOIN hospitals h ON d.hospital_id = h.hospital_id
JOIN wards w ON d.department_id = w.department_id
LEFT JOIN patients p ON w.ward_id = p.current_ward_id
GROUP BY d.department_id, d.name, h.name;
```
## Индексы
### Оптимизация поиска расписания врача
```sql
CREATE INDEX idx_doctor_schedule ON appointments (doctor_id, appointment_date);
```
### Ускорение поиска отзывов по больнице
```sql
CREATE INDEX idx_hospital_reviews ON reviews (hospital_id, rating);
```
## Функции
### Функция 1: Расчет возраста пациента
```sql
...
BEGIN
    ...
    FROM patients 
    WHERE patient_id = calculate_patient_age.patient_id;
    
    RETURN age;
END;
$$ LANGUAGE plpgsql;
```
### Функция 2: Получение списка лекарств пациента
```sql
CREATE OR REPLACE FUNCTION get_patient_medications(p_patient_id INT)
RETURNS TABLE (
...
BEGIN
    ...
    SELECT 
        ...
        m.frequency,
        mr.diagnosis,
        mr.created_at AS prescribed_date
    FROM medications m
    JOIN medical_records mr ON m.record_id = mr.record_id
...
AS $$
BEGIN
    INSERT INTO ward_patient_changes (patient_id, old_ward_id, new_ward_id, changed_by)
    SELECT p_patient_id, current_ward_id, p_new_ward_id, p_changed_by
    FROM patients
    WHERE patient_id = p_patient_id;
    
    UPDATE patients
    SET current_ward_id = p_new_ward_id
    WHERE patient_id = p_patient_id;
...
```
## Тригеры
### Триггер 1: Обновление версии медицинской записи при изменении
```sql
CREATE OR REPLACE FUNCTION update_medical_record_version()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_medical_record_update
BEFORE UPDATE ON medical_records
FOR EACH ROW
EXECUTE FUNCTION update_medical_record_version();
```
### Триггер 2: Проверка вместимости палаты при переводе пациента
```sql
...
BEGIN
    ...
    FROM patients
    WHERE current_ward_id = NEW.new_ward_id;
    
    SELECT capacity INTO ward_capacity
    FROM wards
    WHERE ward_id = NEW.new_ward_id;
    
    IF current_patients >= ward_capacity THEN
        RAISE EXCEPTION 'Палата % уже заполнена (текущее количество пациентов: %, вместимость: %)', 
            NEW.new_ward_id, current_patients, ward_capacity;
    END IF;
...
CREATE TRIGGER trg_check_ward_capacity
BEFORE INSERT ON ward_patient_changes
FOR EACH ROW
EXECUTE FUNCTION check_ward_capacity();
```
### Триггер 3: Логирование изменений специализации врача
```sql
CREATE OR REPLACE FUNCTION log_doctor_specialization_change()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.specialization <> OLD.specialization OR NEW.department_id <> OLD.department_id THEN
        UPDATE doctor_history
        SET valid_to = CURRENT_DATE,
            is_current = false
        WHERE doctor_id = NEW.doctor_id AND is_current = true;
        
        INSERT INTO doctor_history (doctor_id, specialization, department_id, valid_from, is_current)
        VALUES (NEW.doctor_id, NEW.specialization, NEW.department_id, CURRENT_DATE, true);
    END IF;
...
CREATE TRIGGER trg_log_doctor_specialization_change
AFTER UPDATE ON doctors
FOR EACH ROW
WHEN (OLD.specialization IS DISTINCT FROM NEW.specialization OR OLD.department_id IS DISTINCT FROM NEW.department_id)
EXECUTE FUNCTION log_doctor_specialization_change();
```
## Анализ
### Код
```py
...
def generate_and_insert_data():
    ...
    for _ in range(50):
        hospital_id = random.randint(1, 15)
        rating = random.randint(1, 5)
        comment = fake.sentence() if random.random() > 0.3 else None
        reviewer_name = fake.name() if random.random() > 0.2 else None
        
        cursor.execute(
            "INSERT INTO reviews (hospital_id, rating, comment, reviewer_name) VALUES (%s, %s, %s, %s)",
            (hospital_id, rating, comment, reviewer_name)
        )
...
def fetch_data_for_analysis():
    ratings_query = """
    SELECT h.name, h.location, AVG(r.rating) as avg_rating, COUNT(r.review_id) as review_count
    FROM hospitals h
    JOIN reviews r ON h.hospital_id = r.hospital_id
    GROUP BY h.hospital_id
    ORDER BY avg_rating DESC
    """
    
    occupancy_query = """
    SELECT d.name as department, h.name as hospital, 
           SUM(w.capacity) as total_capacity,
           COUNT(p.patient_id) as current_patients,
           ROUND(COUNT(p.patient_id) * 100.0 / NULLIF(SUM(w.capacity), 0), 2) as occupancy_percentage
    FROM departments d
    ...
    JOIN wards w ON d.department_id = w.department_id
    LEFT JOIN patients p ON w.ward_id = p.current_ward_id
    GROUP BY d.department_id, h.hospital_id
    """
    ...
    occupancy_df = pd.read_sql(occupancy_query, conn)
    diagnoses_df = pd.read_sql(diagnoses_query, conn)
    
    return ratings_df, occupancy_df, diagnoses_df

def analyze_and_visualize():
    ratings_df, occupancy_df, diagnoses_df = fetch_data_for_analysis()
    
    plt.figure(figsize=(12, 6))
    sns.barplot(x='avg_rating', y='name', data=ratings_df.sort_values('avg_rating', ascending=False))
    plt.title('Средний рейтинг больниц')
    ...
    sns.scatterplot(x='occupancy_percentage', y='department', hue='hospital', 
                    size='total_capacity', data=occupancy_df)
    plt.title('Заполненность отделений по больницам')
    ...
    sns.barplot(x='count', y='diagnosis', data=diagnoses_df)
    plt.title('Топ-10 самых частых диагнозов')
    plt.xlabel('Количество случаев')
    plt.ylabel('Диагноз')
    plt.tight_layout()
    plt.show()
    
    # Гипотеза 1: Чем больше у больницы рейтинг тем больше отзывов
    corr = ratings_df['avg_rating'].corr(ratings_df['review_count'])
    print(f"Корреляция между рейтингом и количеством отзывов: {corr:.2f}")
    
    # Гипотеза 2: Заполненность хирургических отделений выше чем терапевтических
    surgical = occupancy_df[occupancy_df['department'].str.contains('хирург|Хирург')]['occupancy_percentage'].mean()
    therapeutic = occupancy_df[occupancy_df['department'].str.contains('терапевт|Терапевт')]['occupancy_percentage'].mean()
    print(f"Средняя заполненность хирургических отделений: {surgical:.1f}%")
    print(f"Средняя заполненность терапевтических отделений: {therapeutic:.1f}%")
    
    # Гипотеза 3: Врачи с большим стажем имеют больше назначений
    cursor = conn.cursor()
    cursor.execute("""
    SELECT d.full_name, 
           DATE_PART('year', AGE(NOW(), d.hire_date)) as experience,
    ...
    exp_df = pd.DataFrame(exp_data, columns=['full_name', 'experience', 'appointment_count'])
    exp_corr = exp_df['experience'].corr(exp_df['appointment_count'])
    print(f"Корреляция между стажем врача и количеством назначений: {exp_corr:.2f}")
...
```
### Графики
![./source/hospital_rating.jpg](./source/hospital_rating.jpg)
![./source/department_full.jpg](./source/department_full.jpg)
![./source/diagnos.jpg](./source/diagnos.jpg)
### Результаты 
- Корреляция между рейтингом и количеством отзывов: 0.06
- Средняя заполненность хирургических отделений: 33.3%
- Средняя заполненность терапевтических отделений: 33.3%
- Корреляция между стажем врача и количеством назначений: 0.25
## Выводы
### Рейтинги больниц:
Существует незначительная корреляция между рейтингом и кол-вом отзывов.
### Заполненность отделений:
Гипотеза опровергнута, заполненность равная 
### Стаж врачей
Слабая положительная корреляция (0.25) между стажем врача и количеством назначений действительно существует.
